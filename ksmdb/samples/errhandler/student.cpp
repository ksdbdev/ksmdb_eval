...
bool DB_student::Create(char *path,char *name,size_t size)
{
	...
	m_inited=true;
	if (KsCreateDb(&m_database,path,name,"20210423-10:49:59:000",size) == true)
	{
...
bool DB_student::Open(char *path,char *name,int mode,int port,bool isolate,int reuse_shmmap, int shmrcset_mode)
{
	...
	m_inited=true;
	if (KsOpenDb(&m_database,path,name,"20210423-10:49:59:000",mode,port,isolate, reuse_shmmap, shmrcset_mode) == true)
	{
